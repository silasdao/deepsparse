...
def lambda_handler(event, context):
    """
    This lambda function is triggered by an S3 event when a file
    is added to the specified bucket. The function reads the contents
    of the file, splits it into lines, and submits a batch job
    using AWS Batch service. The batch job runs a Python script
    app.py with environment variables set to the input data in the file.
    ...
    response = batch_client.submit_job(
        ...
        containerOverrides={
            ...
            "environment": [{"name": "INPUTS", "value": ",".join(string_data)}],
        },
    )
